...
namespace Ets2SdkClient.Demo
{
    public partial class Ets2SdkClientDemo : Form
    {
            ...
            try
            {
                ...
                foreach (var grp in grps)
                {
                    ...
                    foreach (var prop in props)
                    {
                        ...
                        if (val is float[])
                        {
                            var str = string.Join(" ", (val as float[]).Select(x => x.ToString("0.000")));
                            vals.AppendLine(str);
                            values.Add(str);
                        }
                        else
                        ...
                        if (csvFile != null && printHeader)
                            csvHeader.Add(grp + "_" + prop.Name);
                    }
                ...
                if (csvFile != null)
                {
                    ...
                    {
                        printHeader = false;
                        csvFile.WriteLine(string.Join(",", csvHeader.ToArray()));
                    }

                    csvFile.WriteLine(string.Join(",", values.ToArray()));
                }
            }
            catch
            {
        ...
        private void export_csv_Click(object sender, EventArgs e)
        {
            if (csvFile != null)
                csvFile.Close();
            ...
            {
                lbl_export.Text = "Export File: " + saveFileDialog.FileName;
                if ((mfile = saveFileDialog.OpenFile()) != null)
                {
                    printHeader = true;
                    csvFile = new StreamWriter(mfile);
